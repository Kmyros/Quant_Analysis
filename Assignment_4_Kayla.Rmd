...
```{r, echo = FALSE}
ggplot(data) +
  geom_point(aes(y = Inv_Sep, x = listP_Sep))
...
## Categorical Variable Test: : T-Test and Regression
...
MBQ_model <- lm(Inv_Sep ~ MortBurd_Quality, data)
...
```{r}
p1 <- ggplot(data) +
  geom_point(aes(x = MedIncome_Quality, y = Inv_Sep))
   p1 + labs (x = "MSA Median Income Quality", y = "Inventory Sep 2021", title = "Regression Analysis")
...
The model below is testing whether or not the Median Sale Price in an MSA's housing market is impacted by the inventory size, list price, rate of mortgage burdened homeowners, and whether or not it is a high median income MSA or a lower median income MSA.  
...
# Questions: it is pulling the Median Income Quality = "Low", how can I change it to high? Same for Mortgage Burden Quality
## low median income and less burdened are the reference groups 
# take out sales price from this model, to see the relationship between the other variables 
```
