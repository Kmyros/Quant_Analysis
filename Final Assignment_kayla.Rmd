...
The unit of our analysis is the Metropolitan Statistical Area, this is the geography level  at whcih the categorical and continious varaibles are available. Initially our data sets only matched by 25 Metropolitan Statistical Areas (MSAs) out of approximately 300 available. There were two issues, the Zillow data sets had only 95 MSAs compared to the 300 listed by the Census Bureau. The second issue, was that the syntax between the data set varied slightly. In order to match the variables, we used the gsub function to rename the Census Data variables to match the Zillow data MSA variables. Then we successfully joined the two data sets based on the re-coded MSA variables. 
